...
function UpcomingItem({ item, navigation }) {
    ...
    const [state, setState] = useState('Chờ giao hàng');

    if (item.state == 2) {
      setState('Đã giao hàng');
    }
    ...
    return(
    <View>
      <Text style={AppStyle.UpcomingItemStyles.id}>#{getRandomInt(100, 999)}</Text>
      <View style={AppStyle.UpcomingItemStyles.container}>
        ...
        <View style={AppStyle.UpcomingItemStyles.status}>
          ...
          <Text style={AppStyle.UpcomingItemStyles.statusContent}>{state}</Text>
        </View>
