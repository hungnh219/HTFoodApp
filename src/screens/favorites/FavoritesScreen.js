...
function FavoritesScreen ({ navigation }) {
  const { productFavoriteList, setProductFavorite, addProductFavorite, removeProductFavorite } = useFavorite();
  ...
  return (
    // <View>
    //   {/* xử lí logic nếu chưa có sản phẩm yêu thích thì màn hình có thông báo giống cart nha */}
    //   <View style={AppStyle.CartStyles.header}>
    //     {/* <TouchableOpacity onPress={() => navigation.goBack()}>
    //       <BackButton />
    //     </TouchableOpacity> */}
    //     <Text style={AppStyle.CartStyles.headerTitle}>Sản Phẩm Yêu Thích</Text>
    //   </View>
    //   <FlatList
    //   data={productFavoriteList.map((item, index) => ({ ...item, index }))}
    ...
    <View>
      <View style={AppStyle.CartStyles.emptyCartContainer}>
        <Text style={AppStyle.CartStyles.emptyCartText}>Sản Phẩm Yêu Thích</Text>
      </View>
      {productFavoriteList.length > 0 ? (
        <FlatList
          data={productFavoriteList.map((item, index) => ({ ...item, index }))}
          keyExtractor={item => item.id.toString()}
          numColumns={1}
